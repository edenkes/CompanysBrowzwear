        ...
        /*
                ...
                CustomersList.forEach(function(element) {
                    ...
                    countriesMap.get(countryName) ?
                        countriesMap.set(countryName, countriesMap.get(countryName) + 1)
                        : countriesMap.set(countryName, 1);
                });
                ...
                countriesMap.forEach((value, key) => {
                    ...
                    countriesList.push({"countryName":key,"count": value, cities: []})
                ...
                CustomersList.forEach(function(element) {
                    ...
                    const companyName = element['CompanyName'];
                    // console.log(element['Country']);
                    CitiesMap.get(countryName) ?
                        CitiesMap.set(countryName, CitiesMap.get(countryName) + 1)
                        : CitiesMap.set(countryName, 1);
                });

                // console.log("sort ", countriesList.sort((first, second) => {return -(first['count'] - second['count'])}));
                countriesList.sort((first, second) => {return -(first['count'] - second['count'])})
                console.log("countryName ", countriesList[0]['countryName']);


                this.setState({
                    countryName: countriesList[0]['countryName'],
                    cityName: countriesList[0]['cities']['cityName'],
                    countriesList: countriesList
                })
        */
            ...
            <Container >
                <Table className="m-4">
                    ...
                    <tr>
                        ...
                        {/*
                            <td>{countriesList[this.state.countryName].map(({CityName}) => (
                                <ul className="list-group">
                                    <Button block className="btn btn-outline-primary border-0">
                                        {CityName}
                                    </Button>
                                </ul>
                            ))}</td>*/}
